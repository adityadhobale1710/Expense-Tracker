...
export const addIncome = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (walletId) {
      const wallet = await Wallet.findOne({ _id: walletId, user: req.user._id }).session(session);
      if (!wallet) {
        res.status(404);
        throw new Error('Wallet not found');
      }
      payload.wallet = wallet._id;
      wallet.balance = Math.round((wallet.balance + Number(rest.amount)) * 100) / 100;
      await wallet.save({ session });
    }

    [income] = await Income.create([payload], { session });
  ...
  try {
    ...
    if (newWalletId && String(oldWalletId) === String(newWalletId)) {
      ...
      if (newAmount !== oldAmount) {
        const wallet = await Wallet.findOne({ _id: newWalletId, user: req.user._id }).session(session);
        if (!wallet) {
          res.status(404);
          throw new Error('Wallet not found');
        }
        if (newAmount > oldAmount) {
          wallet.balance = Math.round((wallet.balance + (newAmount - oldAmount)) * 100) / 100;
        } else {
          const diff = oldAmount - newAmount;
          if (wallet.balance < diff) {
            res.status(400);
            throw new Error('Insufficient wallet balance to reverse this income');
          }
          wallet.balance = Math.round((wallet.balance - diff) * 100) / 100;
        }
        await wallet.save({ session });
      }
    } else {
      ...
      if (oldWalletId) {
        const oldWallet = await Wallet.findOne({ _id: oldWalletId, user: req.user._id }).session(session);
        if (oldWallet) {
          if (oldWallet.balance < oldAmount) {
            res.status(400);
            throw new Error('Insufficient wallet balance to reverse this income');
          }
          oldWallet.balance = Math.round((oldWallet.balance - oldAmount) * 100) / 100;
          await oldWallet.save({ session });
        }
      }
      if (newWalletId) {
        const newWallet = await Wallet.findOne({ _id: newWalletId, user: req.user._id }).session(session);
        if (!newWallet) {
          res.status(404);
          throw new Error('Wallet not found');
        }
        newWallet.balance = Math.round((newWallet.balance + newAmount) * 100) / 100;
...
export const deleteIncome = asyncHandler(async (req, res) => {
  ...
  try {
    const income = await Income.findOneAndDelete({ _id: req.params.id, user: req.user._id }).session(session);
    if (!income) { res.status(404); throw new Error('Income not found'); }

    // Reverse the income addition on the wallet
    if (income.wallet) {
      const wallet = await Wallet.findOne({ _id: income.wallet, user: req.user._id }).session(session);
      if (wallet) {
        const amountToReverse = Number(income.amount || 0);
        if (wallet.balance < amountToReverse) {
          res.status(400);
          throw new Error('Insufficient wallet balance to reverse this income');
        }
        wallet.balance = Math.round((wallet.balance - amountToReverse) * 100) / 100;
        await wallet.save({ session });
      }
    }
    await session.commitTransaction();
