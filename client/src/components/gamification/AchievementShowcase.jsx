  ...
  return (
    <div className="card bg-dark-800 border border-slate-700/50 rounded-2xl p-5 shadow-xl space-y-4">
      <div className="flex justify-between items-center">
        ...
        <span className="text-[9px] font-bold text-slate-500 uppercase tracking-wide">{pinnedAchs.length}/3 Pinned</span>
      </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        {pinnedAchs.map((ach) => (
          <div key={ach.id} className="relative group flex flex-col items-center gap-2 bg-gradient-to-b from-slate-900/50 to-slate-900/20 border border-slate-800/80 rounded-2xl p-3 text-center hover:border-indigo-500/30 transition-all">
            ...
            <div>
              <p className="text-[10px] font-black text-slate-200 leading-tight line-clamp-2">{ach.title}</p>
            ...
            <button
              ...
              className="absolute top-1.5 right-1.5 w-5 h-5 rounded-full bg-slate-900/80 border border-slate-700 text-slate-600 hover:text-red-400 hover:border-red-500/40 opacity-0 group-hover:opacity-100 transition-all text-[9px] flex items-center justify-center cursor-pointer"
              title="Unpin"
            >
              ✕
            </button>
        ...
        {Array.from({ length: 3 - pinnedAchs.length }).map((_, i) => (
          <div key={i} className="flex flex-col items-center justify-center gap-1 border border-dashed border-slate-800/50 rounded-2xl p-3 text-center opacity-40 min-h-[100px]">
            ...
            <p className="text-[9px] text-slate-700 font-bold">Empty Slot</p>
          </div>
        ))}
      </div>
    </div>
